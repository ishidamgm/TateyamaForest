...
#'
BA_matrix <- function(sp="ooshirabiso"){
  BA <- matrix(0,8,6)
  for (ii in 1:8){
    d<-subset(dd3,pn==ii)
    BA[ii,] <- BA_calc(d,sp)
...
#' vital_mortality(dd3,"sugi") #Cryptomeria japonica スギ　####
#' vital_mortality(dd3,"buna") # Fagus crenata ブナ　####
#' vital_mortality(dd3,"ooshirabiso") # Abies mariesii オオシラビソ　####
#'
...
#'
#' for(i in 1:6){
#' dbh_hist("Kagamiishi","ooshirabiso",i,0,seq(0,30,2),ylim=c(0,40))
#' }
#'
