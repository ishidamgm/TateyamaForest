...
# 結果が微妙に違う件
Fig_Abies_wi_ba_mortality()   # Abies wi-ba  y=-0.0993　　r=-0.961
#. <- TemperatureWIAbies_Population_BA_Mortality
(wi.<-subset(TemperatureWIAbies_Population_BA_Mortality,plot=="Kaminokodaira")$WI)
(ba.<-subset(TemperatureWIAbies_Population_BA_Mortality,plot=="Kaminokodaira")$BAratio)
plot(wi.,ba.)
summary(lm(ba.~wi.))

Fig_wi_ba_cor_JVS2()          # Abies wi-ba  y=-0.0914   r=-0.959
subset(TemperatureWIAbiesPopulation,plot=="Kaminokodaira")$WI
# >>>>>>wi は同じ数字を使っている


# Abies wi-ba  y=-0.0914
